...
import { FC, useEffect, useLayoutEffect, useMemo, useReducer, useRef, useState } from 'react';
//@ts-ignore
...
const Debug = () =>{
    ...
    const [update_val,update] = useState<boolean>(false)
	...
	const set = (event: {
		...
		data: any;
		$_update_time: number;
		is_link_component?: string | undefined;
	}) =>{
		const target = data.current.find(elm=> elm.comp_name == event.comp_name && elm.entity == event.entity)
		if(target){
    ...
    useGameEvent("s2c_debug_comp",(event)=>{
      if(event.entity == null) return;
	  	data.current = set(event)
		update(elm=>!elm)
    },[])

	const list = useMemo(()=>{
		return data.current.map((elm:any,index:any)=><Comp key={"comp_debug" + elm.comp_name + elm.entity + elm.$_update_time + Math.random() } index={elm.comp_name + elm.entity } className={elm.comp_name} entity={elm.entity} table={elm.data}/>)
	},[update_val])
    ...
    return   <Panel hittest={false} ref={p=>main.current = p!} style={{flowChildren:"right-wrap",zIndex:100,width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.9)"}}>
...
const Comp = ({className,entity,table,index}:{className:string,entity:number,table:any,index:string}) =>{
    ...
    return  <>
            ...
            <Label style={{flowChildren:"down"}}  text={`<font color='#7FFF00'>[${state ? "close" : "open"}]<font>  <font color='#FFF8DC'>` + "[" + className + "]<font>" + `<font color='#E9967A'>` + "[eid=" + entity + "]<font>"} html={true}>
               {state && <Label style={{"overflow":"scroll",borderRadius:"20px",backgroundColor:"#FFB6C102"}} text={convertJsonToHtml(table)} html={true}/>}
            </Label>
            </>

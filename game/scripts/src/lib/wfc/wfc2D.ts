export namespace WFC {
    ...
    class Slot {
        ...
        public collapseNow(): boolean {
            ...
            let roll = RandomFloat(0,1) * totalP;
            let p = 0;
    ...
    export class WFC2D {
        ...
        public collapseSync(width: number, height: number, backOffMaxNum?: number, capQueueMaxLen?: number, capRate?: number): [string, number][] {
            ...
            this.startTime = GetSystemTimeMS();
            //init
        ...
        public collapse(width: number, height: number,res_fn:Function, backOffMaxNum?: number, capQueueMaxLen?: number, capRate?: number, frameMaxTime?: number): Promise<[string, number][]> {
        ...
        private setCollapseInit(width: number, height: number, backOffMaxNum?: number, capQueueMaxLen?: number, capRate?: number) {
            // tslint:disable-next-line: no-parameter-reassignment
            if (backOffMaxNum == null) { backOffMaxNum = 500; }
            // tslint:disable-next-line: no-parameter-reassignment
            if (capQueueMaxLen == null) { capQueueMaxLen = 10; }
            // tslint:disable-next-line: no-parameter-reassignment
        ...
        private resultCollapse() {
            ...
            this.allSlots.length = 0;
            // tslint:disable-next-line: max-line-length
            print(`collapse complete! total time : ${GetSystemTimeMS() - this.startTime}, solveCount : ${this.solveCount} , backOffCount : ${this.backOffCount}`);
            return result;
        }
        ...
        private calculate(res_fn:Function) {
            return new Promise((resolve, reject) => {
                ...
                let fun = async () => {
                    try{
                        while(!this.isComplete) {
                            print("执行了一次")
                            await this.interval(0.01);
                            this._doCollapse();
                            res_fn(this.solveCount)
                            await fun();
                            return;
                        }
                    }catch(err){
        ...
        private _doCollapse() {
            ...
            if (!isFirst) {
                ...
                let rIdx = Math.floor(RandomFloat(0,1) * this.allSlots.length);

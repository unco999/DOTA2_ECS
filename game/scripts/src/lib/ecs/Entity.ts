...
export class Entity implements ReadonlyEntity {
  ...
  public addComponent<T extends K, K extends unknown>(component: NonNullable<T>, resolveClass?: Class<K>): void {
    ...
    if (this._components[id - 1] !== undefined) {
      if (!linkedComponent && component === this._components[id - 1]) {
    ...
    if (linkedComponent) {
      ...
      this._components[id - 1] = component;
  ...
  public addTag(tag: Tag): void {
    if (!this._tags.has(tag)) {
      ...
      if(player_id == null){
         ...
         CustomNetTables.SetTableValue("system_tag",tag.toString(), obj ? Object.assign(obj,{[tag]:1}) : {[tag]:1})
      }
      if(player_id){
         print("设置了新的tag","player" + player_id)
         DeepPrintTable({[tag]:1});
         const obj = CustomNetTables.GetTableValue("player_tag","player" + player_id)
         CustomNetTables.SetTableValue("player_tag","player" + player_id, obj ? Object.assign(obj,{[tag]:1}) : {[tag]:1})
      }
    }
  }
  ...
  public get<T>(componentClass: Class<T>, id?: string): T | undefined {
    ...
    let component = this._components[cid - 1];
    if (id !== undefined) {
      if (isLinkedComponent(component)) {
    ...
    return this._components[cid - 1] as T;
  }
  ...
  public removeComponent<T>(componentClassOrTag: Class<T>): T | undefined {
    ...
    let value = this._components[id - 1]!;
  ...
  public removeTag(tag: Tag): void {
    if (this._tags.has(tag)) {
      ...
      if(player_id){
        const obj = CustomNetTables.GetTableValue("player_tag","player" + player_id)
        CustomNetTables.SetTableValue("player_tag","player" + player_id, obj ? Object.assign(obj,{[tag]:0}) : {[tag]:0})
      }
    }
  }
  ...
  public copyFrom(entity: Entity): this {
    this._components = table.deepcopy(entity._components) as any[]
    this._linkedComponents =table.deepcopy(entity._linkedComponents) as any[]
  ...
  public find<T>(componentClass: Class<T>, predicate: (component: T) => boolean): T | undefined {
    ...
    const component = this._components[componentIdToFind - 1];
  ...
  public takeSnapshot<T>(result: EntitySnapshot, changedComponentOrTag?: T, resolveClass?: Class<T>): void {
    ...
    if (isTag(changedComponentOrTag)) {
      ...
      const componentClass = resolveClass ?? changedComponentOrTag.constructor as Class<T>;
      const componentId = getComponentId(componentClass!, true)!;
 ...
 * previous state.
 *
 * The  property always reflects the current state, and  -

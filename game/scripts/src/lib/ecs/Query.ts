...
export class QueryBuilder {
   ...
   */
  public contains<T extends keyof Entity>(...componentsOrTags: Array<any | T > | T[] ): QueryBuilder {
    for (const componentOrTag of componentsOrTags) {
      if (isTag(componentOrTag)) {
        ...
        const componentId = getComponentId(componentOrTag, true)! - 1;
        print("当前查询的id",componentId)
